...
background-repeat:no-repeat;width:1400px; }
...
<form method="post" action="forecast-2.php">
Show: <select style="width:60px;" name="limit" onchange="this.form.submit()">
  ...
  <tr>
	...
	<label for="INTERNAL_ORDER">&nbsp; Internal Order<span></span></label>
	  <input type="text" name="var[INTERNAL_ORDER]" value="<?php echo $varINTERNAL_ORDER; ?>" onChange="this.form.submit()">
	</th>
	...
	<th>Rate Code</br>
	  <select style="margin:5px 0 0 0;" name="var[RATE_CODE]" onchange="this.form.submit()">
			<option value=""></option>
	...
	<th>Designer</br>
	  <select style="margin:5px 0 0 0;" name="var[DESIGNER]" onchange="this.form.submit()">
		<option value=""></option>
		<?php 
		
		  $result9= $mysqli->query("Select NAME from Users where ORG in ('Keith Hester')
	    and NAME!='Adam' order by NAME");
	    while($row9= $result9->fetch_assoc()) {
			
			  $ownerOption= $row9['NAME'];
				echo '<option value="'.$ownerOption.'"';
				if($ownerOption==$varNCS_OWNER) { echo ' selected="true"'; }
				echo '>'.$ownerOption.'</option>';
			}
		?>
		</select>
	</th>
	...
	<th>Provisioner</br>
	  <select style="margin:5px 0 0 0;" name="var[PROVISIONER]" onchange="this.form.submit()">
		<option value=""></option>
		<?php 
		
		  $result9= $mysqli->query("Select NAME from Users where ORG in ('Tony Marler')
	    and NAME!='Adam' order by NAME");
	    while($row9= $result9->fetch_assoc()) {
			
			  $ownerOption= $row9['NAME'];
				echo '<option value="'.$ownerOption.'"';
				if($ownerOption==$varPROV_OWNER) { echo ' selected="true"'; }
				echo '>'.$ownerOption.'</option>';
			}
		?>
		</select>
  ...
  <th>
	  <input type="checkbox" id="NEW_ACTL" name="sort[NEW_ACTL]" onchange="this.form.submit()" 
		<?php if($sortField=='NEW_ACTL') { echo ' checked'; } ?>>
	<label for="NEW_ACTL">&nbsp; New ACTL<span></span></label>
	</th>
...
<?php
  ...
  $e=0;
	...
	$query= "Select ID,VENDOR_CKT_CLN, INTERNAL_CKT_CLN, INTERNAL_ORDER, SAVINGS, RATE_CODE, DESIGNER, PROVISIONER, SUBPROJECT, STATUS, INITIAL_FOC, INTERNAL_ORDER, LATA, NEW_ACTL from Simple where 1 $search $sort limit $limit";
    ...
    echo '</a></td>';
		echo '<td style="min-width:120px;max-width:240;text-align:center;">'. $row['INTERNAL_ORDER'] .'</td>';
		...
		echo '<td style="width:100px;min-width:100px;max-width:100px;text-align:center;">'. $row['DESIGNER'] .'</td>';
		echo '<td style="width:100px;min-width:100px;max-width:100px;text-align:center;">'. $row['PROVISIONER'] .'</td>';
		...
		echo '<td style="width:100px;min-width:100px;max-width:100px;text-align:center;">'. $row['NEW_ACTL'] .'</td>';
