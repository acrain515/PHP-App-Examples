...
<?php 
  ...
  $first= date("Y-m-01 00:00:00",$firstStr);
	...
	$filename = 'mileage-chart-g-source.php';
  ...
  //fwrite($myfile, "<?php \n\n");
    ...
    echo 'Current Count: ' . $totalCount . '</br>';
		...
		foreach($AxcentDS1CutsCount as $key=>$value) {
		echo 'Key: ' . $key . '</br>';
		  if($month==$key) { fwrite($myfile,$value); }
		}
		fwrite($myfile,",");
		foreach($WinDS1CutsCount as $key=>$value) {
		  echo 'Key: ' . $key . '</br>';
		  if($month==$key) { fwrite($myfile,$value); }
		}	
	...
	nextUrl("mileage-chart-h-write.php");
