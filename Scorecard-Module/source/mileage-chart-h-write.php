<?php 
  ...
  $first= date("Y-m-01 00:00:00",$firstStr);
	...
	$filename = 'mileage-chart-h-source.php';
  ...
  //fwrite($myfile, "<?php \n\n");
    ...
    echo 'Current Count: ' . $count . '</br>';
	  ...
	  $savings= $row['SAVINGS'];
		...
		$totalSavings= $savings+$savings2;

    if($count==0) { $average= 0; }	
    else {	$average= round($totalSavings/$totalCount); }
		...
		foreach($DS1CutsAvgSave as $key=>$value) {
		echo 'Key: ' . $key . '</br>';
	...
	nextUrl("mileage-chart-i-write.php");
