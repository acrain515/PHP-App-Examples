...
//SET VARIABLES FOR OVERALL DOC
  ...
  $bookTitle="Tony-Pending-Ready-PAE";
  //$sys= "PAE";
  //$parent= "('NETSVG','SVGSPRV')";
...
//SET ARRAY FOR COL LETTERS
  ...
  $excel->getProperties()
	...
	->setTitle("Tony Pending & Ready PAE")
	->setSubject("")
	->setDescription("")
	->setKeywords("")
	->setCategory("");
  ...
  $result= $local->query("Select * from Tasks_PAE where STATUS='Ready' 
  and (PARENT_QUEUE='NETSVG' or PARENT_QUEUE='SVGSPRV') 
  group by TASK");
...
//GET ALL THE QUEUES IN THE PARENT QUEUE WITH PENDING DD TASKS OR PENDING/READY OTHER TASKS
  $result2= $local->query("Select * from Tasks_PAE where (STATUS='Ready' or (TASK='DD' and STATUS='Pending'))
  and (PARENT_QUEUE='NETSVG' or PARENT_QUEUE='SVGSPRV')
  group by QUEUE");
...
//GET THE COUNT OF THE PENDING DD TASKS IN EACH QUEUE 	
	$result3= $local->query("Select count(*) as theCount from Tasks_PAE where TASK='DD' and STATUS='Pending' and QUEUE='$queue'");
...
//GET THE COUNTS FOR EACH READY TASK IN EACH QUEUE	
	...
	for($x=0;$x<$taskCount;$x++) {
      ...
      $result4= $local->query("Select *,count(*) as theCount from Tasks_PAE where STATUS='READY' and QUEUE='$queue' and TASK='$task'");
...
//HEADERS
  ...
  $vCol= "A";
  $result5= $local->query("Show columns from Tasks_PAE");
...
//GET CONTENT 
  $x=1;
  $result5= $local->query("Select * from Tasks_PAE where (STATUS='Ready' or (TASK='DD' and STATUS='Pending'))
  and (PARENT_QUEUE='NETSVG' or PARENT_QUEUE='SVGSPRV') 
  ");
  while ($row5= $result5->fetch_assoc()) {
    ...
    $result6= $local->query("Select ORDER_NUMBER from Service_Requests_PAE where DOCUMENT_NUMBER='$doc'");
