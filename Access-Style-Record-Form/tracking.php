...
<?php 
  ...
  //FILTER CAPABILITY FIELDS ARRAY	
	$filterFields= array("CUSTOMER_NAME","LEC_CKT_CLN","INTERNAL_CKT","CLLI","RESEARCHER",
...
<?php 
  ...
  if(isset($_POST['filter'])) { 
	  $filterField= $_POST['filterField'];
		...
		if(strpos($search,$filterField)!==false) { $search= ""; }
		...
		else if($filterType=='in') { $search.= " and " . $filterField . " in (" . $filterValue . ")"; }
	}
  ...
  if(isset($_POST['idString'])) { 
	...
	echo '<a style="font-weight:300;text-decoration:underline;" href="tracking.php">Remove filters</a></br>';
...
<!-- LEC ID AND MRC INFO -->
      ...
      <tr>
			  <td colspan="2" >LEC Circuit ID
				...
				<span class="toggleLEC_CKT_CLN"><img src="filter-icon.png"
	      <?php if(strpos($search,'LEC_CKT_CLN')==false) { echo ' style="opacity:0.4;"'; } ?>
	      border="0"></span>
				</td>
...
<table>
<tr>
  <td style="min-width:180px;">Internal Circuit ID</br><?php showForm($row,"INTERNAL_CKT","y",$search,"width:160px;");  echo "\n\n"; ?></td>
  <td>Rate Code</br><?php showForm($row,"RATE_CODE","n",$search,"width:40px;");  echo "\n\n"; ?></td>
	<td>Inv Sys</br><?php showFormSelect($row,"INV_SYS","n",$search,"width:80px;",$invSysOptions);  echo "\n\n"; ?></td>
</tr>

<tr>  
<td>Inv Account</br><?php showForm($row,"INV_ACCT","y",$search,"width:160px;");  echo "\n\n"; ?></td>
<td>Int Circuit Status</br><?php showFormSelect($row,"INV_CKT_STATUS","n",$search,"width:110px;",$statusOptions);  echo "\n\n"; ?>
<td>Inv Acct Status</br>
<?php showFormSelect($row,"INV_ACCT_STATUS","n",$search,"width:80px;",$statusOptions);  echo "\n\n"; ?></td>	
</tr>
<tr>
<td>Disc Serv Req</br>
<?php showForm($row,"DISC_SR","n",$search,"width:128px;");  echo "\n\n"; ?></td>
<td>Disc SR Date</br>
				<?php showForm($row,"DISC_SR_DATE","n",$search,"width:60;");  echo "\n\n"; ?></td>
				<td>Disc SR User</br>
				<?php showForm($row,"DISC_SR_USER","n",$search,"width:80px;");  echo "\n\n"; ?></td>
...
<input type="submit" value="ADD">
    ...
    <table style="border:0;">
      <tr>
			  <td>Disc Order User ID</br><?php showForm($row,"DISC_USER","n",$search,"X");  echo "\n\n"; ?></td>
				...
				<td>Disc Status</br><?php showFormSelect($row,"DISC_STATUS","n",$search,"X",$discStatusOptions);  echo "\n\n"; ?></td>
			</tr>
			<tr>
 ...
 <?php 
    ...
    <table style="border:0;">
		  <tr>
			  <td>Original LEC Order</br><?php showForm($row,"DISPUTE_ORIGINAL_LEC_ORDER","n",$search,"X");  echo "\n\n"; ?></td>
				<td>Original PON</br><?php showForm($row,"DISPUTE_ORIGINAL_PON","n",$search,"X");  echo "\n\n"; ?></td>
				<td>Original FOC Date</br><?php showForm($row,"DISPUTE_ORIGINAL_FOC_DATE","n",$search,"X");  echo "\n\n"; ?></td>
				<td></td>
			</tr>
		  <tr>
			  <td>Disc Order User ID</br><?php showForm($row,"DISC_USER","n",$search,"X");  echo "\n\n"; ?></td>
    ...
    <table style="border:0;">
      <tr>
				...
				<?php showForm($row,"BILLING_UPDATED","n",$search,"X");  echo "\n\n"; ?></td>
				<input type="text" name="currentMRR" value=""></td>
				<td>MRR $ Added</br><?php showForm($row,"BILL_DIFF","n",$search,"X");  echo "\n\n"; ?></td>
			</tr>
		</table>
	</div>
	<?php } ?>
...
<?php showFormSelect($row,"PROCESS_FAILURE","n",$search,"X",$failureOptions);  echo "\n\n"; ?></td>
<!--RESEARCH STATUS-->
