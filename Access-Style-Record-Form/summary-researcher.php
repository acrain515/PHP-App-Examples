...
<span style="font-size:9px;"><i>may take a few seconds to load completely</i></span></br>
<table>
<tr>
<th>Researcher</th><th>Circuit Count</th><th>Sum MRC</th>
<th>Pend Review</th><th>Pend Review MRC</th><th>Go to Form</th>
</tr>
<?php  
  ...
  require_once('../connection.php');
	  ...
	  $result= $mysqli->query("Select RESEARCHER from Dbb group by RESEARCHER");
		while($row= $result->fetch_assoc()) { 
		
		  $researcher= $row['RESEARCHER'];
			
			$result2= $mysqli->query("Select count(*) as COUNT,sum(MRC) as SUM from Dbb where RESEARCHER='$researcher'");
			...
			echo '<input type="hidden" name="filterField" value="RESEARCHER">';
			echo '<input type="hidden" name="filterValue" value="'.$researcher.'">';
			echo '<td style="text-align:center;">';
